# ===== import 區域 =====
from flask import Flask, request, abort
import os
import time
import json
import math
import re
from collections import Counter
from openai import OpenAI

from linebot import LineBotApi, WebhookHandler
from linebot.exceptions import InvalidSignatureError
from linebot.models import MessageEvent, TextMessage, TextSendMessage
# ===== import 區域結束 =====


# ===== 多語系文字（固定字串，不交給 GPT）=====
TEXTS = {
    "zh": {
        "intro": (
            "你好，我是【微轉人資AI助手】🤖\n\n"
            "你可以直接輸入人資相關問題，例如：\n"
            "・病假規則是什麼？\n"
            "・加班費如何計算？\n"
            "・特休規定有哪些？\n"
        ),
        "prefix": "📌 根據 {month} 的 {code} 版本《{name}》內容回覆：\n\n",
        "fallback": "此問題在目前規章引用內容中找不到明確依據，請洽人資專員。",
        "answer_label": "【HR AI 助手回覆】\n\n",
        "disclaimer": (
            "\n\n——\n"
            "⚠️【免責聲明】\n"
            "本回覆由微轉人資 AI 助理依據現行內部規章文件摘要提供，僅供參考。\n"
            "如有疑慮或需正式認定，請洽人資專員並以公司公告為準。"
        ),
        "lang_rule": "請用繁體中文回覆。"
    },
    "en": {
        "intro": (
            "Hi, I'm the microSHIFT HR AI assistant 🤖\n\n"
            "You can ask HR-related questions, for example:\n"
            "• Sick leave policy\n"
            "• Overtime pay calculation\n"
            "• Annual leave rules\n"
        ),
        "prefix": "📌 Based on {month} version {code} “{name}”, reply:\n\n",
        "fallback": "I can’t find a clear basis in the current HR policy excerpts. Please contact HR.",
        "answer_label": "【HR AI Assistant】\n\n",
        "disclaimer": (
            "\n\n—\n"
            "⚠️ Disclaimer\n"
            "This reply is generated by an HR AI assistant based on internal policy excerpts for reference only.\n"
            "For official confirmation, please contact HR and refer to company announcements."
        ),
        "lang_rule": "Please reply in English."
    },
    "vi": {
        "intro": (
            "Xin chào, tôi là trợ lý AI Nhân sự microSHIFT 🤖\n\n"
            "Bạn có thể hỏi các câu hỏi liên quan đến Nhân sự, ví dụ:\n"
            "• Quy định nghỉ ốm\n"
            "• Cách tính lương tăng ca\n"
            "• Quy định nghỉ phép năm\n"
        ),
        "prefix": "📌 Dựa trên phiên bản {month} {code} “{name}”, trả lời:\n\n",
        "fallback": "Tôi không tìm thấy căn cứ rõ ràng trong trích đoạn quy định hiện tại. Vui lòng liên hệ Nhân sự.",
        "answer_label": "【Trợ lý AI Nhân sự】\n\n",
        "disclaimer": (
            "\n\n—\n"
            "⚠️ Tuyên bố miễn trừ trách nhiệm\n"
            "Câu trả lời này do trợ lý AI Nhân sự tạo dựa trên trích đoạn quy định nội bộ và chỉ mang tính tham khảo.\n"
            "Để xác nhận chính thức, vui lòng liên hệ Nhân sự và theo thông báo của công ty."
        ),
        "lang_rule": "Vui lòng trả lời bằng tiếng Việt."
    }
}


def detect_lang(text: str) -> str:
    """
    簡易語言判斷（不額外呼叫 GPT，省錢也更穩）：
    - 有越南文變音符號 → vi
    - 英文字母比例高 → en
    - 其他 → zh
    """
    t = (text or "").strip()
    if not t:
        return "zh"

    if re.search(r"[ăâđêôơưáàảãạấầẩẫậắằẳẵặéèẻẽẹếềểễệíìỉĩịóòỏõọốồổỗộớờởỡợúùủũụứừửữựýỳỷỹỵ]", t, re.IGNORECASE):
        return "vi"

    letters = re.findall(r"[A-Za-z]", t)
    if len(letters) >= 6 and (len(letters) / max(len(t), 1)) > 0.25:
        return "en"

    return "zh"


# ===== HR Bot 設定 =====
INTRO_COOLDOWN_SECONDS = 60 * 60 * 12  # 12 小時
last_seen = {}  # 暫存每個 LINE 使用者的最後互動時間（注意：Render 重啟會清空，屬正常）
SIMILARITY_THRESHOLD = 0.28  # 太常拒答可調低 0.24~0.26
TOP_K = 6  # 抓回來的引用段落數


# ===== 建立 Flask / OpenAI / LINE 物件 =====
app = Flask(__name__)
client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
line_bot_api = LineBotApi(os.getenv("LINE_CHANNEL_ACCESS_TOKEN"))
handler = WebhookHandler(os.getenv("LINE_CHANNEL_SECRET"))


# ===== 讀取 HR KB Index（啟動時讀一次）=====
KB_INDEX_PATH = os.path.join(os.path.dirname(__file__), "hr_kb_index.json")
with open(KB_INDEX_PATH, "r", encoding="utf-8") as f:
    KB = json.load(f)

KB_ITEMS = KB.get("items", [])
# KB["policies"] 可選用，這裡不一定要


# ===== 向量相似度 =====
def cosine_sim(a, b):
    dot = 0.0
    na = 0.0
    nb = 0.0
    for x, y in zip(a, b):
        dot += x * y
        na += x * x
        nb += y * y
    if na == 0 or nb == 0:
        return 0.0
    return dot / (math.sqrt(na) * math.sqrt(nb))


def retrieve_chunks(query: str, top_k: int = TOP_K):
    q_emb = client.embeddings.create(
        model="text-embedding-3-small",
        input=query
    ).data[0].embedding

    scored = []
    for it in KB_ITEMS:
        emb = it.get("embedding")
        txt = (it.get("text") or "").strip()
        if not emb or not txt:
            continue
        s = cosine_sim(q_emb, emb)
        scored.append((s, it))

    scored.sort(key=lambda x: x[0], reverse=True)
    top = scored[:top_k]

    refs = []
    for s, it in top:
        refs.append({
            "score": float(s),
            "policy_month": it.get("policy_month", "未知月份"),
            "policy_code": it.get("policy_code", "未知版次"),
            "policy_name": it.get("policy_name", "未知辦法"),
            "source_filename": it.get("source_filename", ""),
            "chunk_id": it.get("chunk_id", 0),
            "text": (it.get("text", "") or "").strip()
        })
    return refs


def pick_best_policy(refs):
    """
    以多數決選最像哪一份文件；平手就取第一個（通常是最高分）
    """
    if not refs:
        return ("未知月份", "未知版次", "未知辦法")
    keys = [(r["policy_month"], r["policy_code"], r["policy_name"]) for r in refs]
    c = Counter(keys)
    best_key, _ = c.most_common(1)[0]
    return best_key


# ===== callback Webhook 接收 =====
@app.route("/callback", methods=["POST"])
def callback():
    signature = request.headers.get("X-Line-Signature")
    body = request.get_data(as_text=True)

    try:
        handler.handle(body, signature)
    except InvalidSignatureError:
        abort(400)

    return "OK"


# ===== handler：HR + RAG + GPT =====
@handler.add(MessageEvent, message=TextMessage)
def handle_message(event):
    user_text = (event.message.text or "").strip()
    user_id = event.source.user_id

    # ---- 語言判斷 ----
    lang = detect_lang(user_text)
    T = TEXTS.get(lang, TEXTS["zh"])

    # ---- intro 顯示節奏 ----
    now = time.time()
    last = last_seen.get(user_id)
    should_show_intro = (last is None) or ((now - last) > INTRO_COOLDOWN_SECONDS)
    last_seen[user_id] = now

    # ---- RAG 檢索 ----
    refs = retrieve_chunks(user_text, top_k=TOP_K)
    best_score = refs[0]["score"] if refs else 0.0

    # 相似度不足：直接拒答（避免「泛勞基法」或亂補）
    if best_score < SIMILARITY_THRESHOLD:
        core = T["fallback"]
        if should_show_intro:
            reply_text = f"{T['intro']}\n{core}{T['disclaimer']}"
        else:
            reply_text = f"{core}{T['disclaimer']}"

        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text=reply_text)
        )
        return

    # ---- 版本前綴 ----
    policy_month, policy_code, policy_name = pick_best_policy(refs)
    prefix = T["prefix"].format(month=policy_month, code=policy_code, name=policy_name)

    # ---- 引用內容區塊（只給必要片段）----
    context_block = "\n\n".join(
        [f"[{r['policy_code']}#{r['chunk_id']}] {r['text']}" for r in refs]
    )

    # ---- GPT Prompt：強制只依引用內容 + 指定回覆語言 ----
    prompt = f"""
你是 microSHIFT 公司的 HR 人資助理。

【規則（非常重要）】
1) 你只能根據下方【引用內容】回答，禁止使用一般常識、網路資訊或推測補充。
2) 如果【引用內容】不足以回答，請只回覆下列句子（不要加任何補充）：
「{T['fallback']}」

【語言要求】
{T['lang_rule']}

【引用內容】
{context_block}

【員工問題】
{user_text}

【回答格式要求】
- 專業、清楚、簡短
- 優先用條列
- 若有條件/例外，需講清楚
"""

    resp = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "你是公司內部 HR Bot（只能依引用內容回答）"},
            {"role": "user", "content": prompt}
        ],
        temperature=0.1
    )

    gpt_answer = (resp.choices[0].message.content or "").strip()

    # ---- 組合回覆（免責聲明固定加在最後，不交給 GPT）----
    if should_show_intro:
        reply_text = f"{T['intro']}\n{prefix}{T['answer_label']}{gpt_answer}{T['disclaimer']}"
    else:
        reply_text = f"{prefix}{T['answer_label']}{gpt_answer}{T['disclaimer']}"

    line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(text=reply_text)
    )


if __name__ == "__main__":
    port = int(os.environ.get("PORT", 10000))
    app.run(host="0.0.0.0", port=port)
